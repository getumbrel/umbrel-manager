...
async function getAvailableUpdate() {
  try {
    ...
    while (isNewVersionAvailable && !isCompatibleWithCurrentVersion) {
      ...
      const latestVersionInfo = await fetch(infoUrl); // eslint-disable-line no-await-in-loop
...
async function getUpdateStatus() {
  try {
    const status = await diskLogic.readStatusFile('update-status.json');
    return status;
  } catch {
...
async function startUpdate() {
  ...
  try {
    const updateStatus = await diskLogic.readStatusFile('update-status.json');
  ...
  try {
    await diskLogic.writeSignalFile('update');
...
async function getBackupStatus() {
  try {
    const status = await diskLogic.readStatusFile('backup-status.json');
    return status;
  } catch {
...
async function requestShutdown() {
  try {
    await diskLogic.writeSignalFile('shutdown');
...
async function requestReboot() {
  try {
    await diskLogic.writeSignalFile('reboot');
